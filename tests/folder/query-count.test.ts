...
import { createFolder, getSubtree, getWorkspaceFolders } from "@/lib/closure";
...
describe("getSubtree — fixed SQL statement count regardless of subtree depth (TREE-02)", () => {
  const createdWorkspaces: string[] = [];
  ...
  it("issues the same statement count for a depth-2 and a depth-6 subtree", async () => {
    await debugDb.execute(sql`SELECT 1`);

    const wsShallow = await createTestWorkspace("query-count-subtree-shallow-ws");
    createdWorkspaces.push(wsShallow.id);
    const root1 = await createFolder(wsShallow.id, null, "root");
    let parent = root1.id;
    for (let depth = 0; depth < 2; depth++) {
      const created = await createFolder(wsShallow.id, parent, `depth-${depth}`);
      parent = created.id;
    ...
    queryCount = 0;
    await getSubtree(root1.id, debugDb);
    const shallowCount = queryCount;

    const wsDeep = await createTestWorkspace("query-count-subtree-deep-ws");
    createdWorkspaces.push(wsDeep.id);
    const root2 = await createFolder(wsDeep.id, null, "root");
    parent = root2.id;
    for (let depth = 0; depth < 6; depth++) {
      const created = await createFolder(wsDeep.id, parent, `depth-${depth}`);
      parent = created.id;
    ...
    queryCount = 0;
    await getSubtree(root2.id, debugDb);
    const deepCount = queryCount;
