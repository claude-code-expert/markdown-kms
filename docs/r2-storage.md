...
문서 편집 화면에서 툴바의 **클라우드에 이미지 업로드**(구름 아이콘, 이미지 아이콘 바로 오른쪽)를 누르고 PNG를 하나 고른다. 에디터에 파일을 드래그해도 같다.
...
역할이 완전히 다르다. 이름이 비슷해 헷갈리기 쉬우므로 명확히 해둔다.

| 버튼 | 하는 일 | 파일 업로드 |
|---|---|---|
| **이미지 삽입** (사각형 아이콘) | 마크다운 문법 `![alt](url)`을 넣는다. 링크 버튼과 같은 서식 버튼이다 | **없음** |
| **클라우드에 이미지 업로드** (구름 아이콘) | 파일을 R2에 올리고 그 URL을 본문에 넣는다 | R2 |

이미 인터넷 어딘가에 있는 이미지를 참조할 때는 앞쪽을, 내 파일을 올릴 때는 뒤쪽을 쓴다.

파일 업로드 경로는 **R2 하나뿐이다.** 서버 로컬 디스크에 저장하던 예전 경로(`POST /api/uploads`, `saveUpload`)는 제거했다 — 서버리스 파일시스템이 읽기 전용이라 프로덕션에서 동작하지 않았고, 정적 서빙 경로에 권한 검증도 없었다.

에디터 영역에 파일을 **드래그 앤 드롭**해도 같은 R2 경로로 올라간다.
...
7. **버튼을 헷갈린다.** "이미지 삽입"은 `![alt](url)` 문법만 넣는 서식 버튼이라 파일이 올라가지 않는다. 파일을 올리려면 **구름 아이콘**을 누르거나 에디터에 드래그한다.
