...
const ACCOUNT_COLUMNS = { id: user.id, email: user.email, name: user.name } as const;
...
export async function createUserInDefaultWorkspace(
  tx: DbClient,
  values: { email: string; name: string; passwordHash: string | null },
): Promise<AccountRow> {
  const workspaceId = await defaultWorkspaceId(tx);

  const [newUser] = await tx
    .insert(user)
    .values({ email: values.email, name: values.name, passwordHash: values.passwordHash })
...
/**
 * Google 로그인용. 이메일이 이미 있으면 **그 계정을 그대로 재사용**하고(= 계정 자동 연결,
 * passwordHash를 건드리지 않는다), 없으면 OAuth 전용 계정으로 새로 만든다.
 *
 * 자동 연결은 호출 전에 isVerifiedGoogleProfile()로 email_verified를 확인했다는 전제 위에서만
 * 안전하다. 남은 잔여 위험(비밀번호 가입에 이메일 인증이 없어 생기는 선점)은
 * docs/oauth-google.md "알려진 미해결 이슈"에 수용된 리스크로 기록돼 있다.
 */
export async function findOrCreateOAuthUser(values: {
  ...
  return db.transaction(async (tx) => {
    ...
    const [inserted] = await tx
      .insert(user)
      .values({ email: values.email, name: values.name, passwordHash: null })
