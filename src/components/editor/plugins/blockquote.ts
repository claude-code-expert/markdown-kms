...
// Per-line, toggle-off on re-application, multi-line re-prefixes each line.
...
function run(state: EditorState): TransactionSpec {
  return state.changeByRange((range) => {
    ...
    // assoc +1: keep a line-start / empty-line caret AFTER the inserted '> ' prefix rather than
    // in front of it (default assoc -1 leaves it before the marker — Phase 2 UAT caret bug).
