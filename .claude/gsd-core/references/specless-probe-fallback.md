...
_gsd_lib() { for _d in "$_GSD_RT/gsd-core/bin/lib" "$_GSD_RT/bin/lib" "$_GSD_RT/.claude/bin/lib" "/Users/codevillain/Claude-Code-Expert/markdown-kms/.claude/gsd-core/bin/lib" "/Users/codevillain/Claude-Code-Expert/markdown-kms/.claude/bin/lib"; do [ -f "$_d/$1" ] && { echo "$_d/$1"; return; }; done; }
...
# consuming project's git root — falling back to git toplevel / /Users/codevillain/Claude-Code-Expert/markdown-kms/.claude (spec-phase.md:198 idiom).
_GSD_RT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"
EDGE_PROBE_JS=$(for _c in \
  "$_GSD_RT/gsd-core/bin/lib/edge-probe.cjs" "$_GSD_RT/bin/lib/edge-probe.cjs" \
  "$_GSD_RT/.claude/bin/lib/edge-probe.cjs" "/Users/codevillain/Claude-Code-Expert/markdown-kms/.claude/gsd-core/bin/lib/edge-probe.cjs" \
  "/Users/codevillain/Claude-Code-Expert/markdown-kms/.claude/bin/lib/edge-probe.cjs"; do [ -f "$_c" ] && { echo "$_c"; break; }; done)
...
`/Users/codevillain/Claude-Code-Expert/markdown-kms/.claude/gsd-core/references/prohibition-probe.md` (do not inline it). Summary:
